...
**In scope:**
- `GenerateContentResponse`-based calls only — i.e. everything that already flows through
...
public sealed class GeminiCostGovernor : ICostGovernor
{
    ...
    //   i.e. "disabled" means "no enforcement," matching the existing IRateLimiter.CreateDisabled()
...
keeps it well under any reasonable limit — prefer the existing file).
...
public class BudgetOptions
{
    ...
    /// snapshot of Google's published rates at the time this library version shipped — WILL go stale;
    /// override here for accuracy) for any model not present in this dictionary. Keyed by the exact
