...
already no-op internally when `Enabled` is `false` — no allocation to avoid either; `TagList`'s
inline storage covers our handful of tags with no heap allocation regardless. What the guard
actually skips is smaller than that: the (cheap, non-allocating) work of building the `TagList` and
entering a call that was going to immediately no-op anyway.
...
that's already reasonably tight: one small, honest fix — corrected once already above, which is
the point of writing this down rather than just asserting it — not a dramatic before/after
headline.
